...
function AdminDashboardPage() {
    const [userData, setUserData] = useState({ full_name: 'Usu├írio' });
    ...
    const months = [
        ...
        { val: 0, label: "Janeiro" }, { val: 1, label: "Fevereiro" }, { val: 2, label: "Mar├ºo" },
    ...
    const chartData = {
        ...
        datasets: [
            {
                label: selectedMonth === -1 ? 'Fluxo de Question├írios' : `Question├írios (${months.find(m => m.val === selectedMonth)?.label})`,
            ...
            {
                label: selectedMonth === -1 ? 'Novos Question├írios' : 'Novos Question├írios',
        ...
        <Box sx={{ p: { xs: 2, md: 4 }, bgcolor: '#f4f6f8', minHeight: '100vh' }}>
            <Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" alignItems="center" spacing={2} sx={{ mb: 4 }}>
                <Box sx={{ flex: 1, minWidth: { xs: '100%', md: 'auto' } }}>
                    <Typography variant="h4" sx={{ fontWeight: 800, color: '#1b5e20', fontSize: { xs: '1.75rem', sm: '2.125rem', lg: '2.5rem' } }}>Ol├í, {userData?.full_name?.split(' ')[0] || 'Gestor'}!</Typography>
            ...
            <Grid container spacing={{ xs: 3, md: 5 }} rowSpacing={8} alignItems="stretch">
                    ...
                    <Paper sx={{ 
                        ...
                        <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
                            <Box sx={{ overflow: 'hidden' }}>
                                <Typography variant="overline" sx={{ color: '#64748b', fontWeight: 800, letterSpacing: 1.5, fontSize: '0.65rem', display: 'block', whiteSpace: 'nowrap' }}>ATIVIDADE M├ëDIA</Typography>
                ...
                <Grid item xs={12} sm={6} md={3}>
                    <Paper sx={{ 
                        ...
                        <Stack direction="row" justifyContent="space-between" alignItems="flex-start">
                            <Box sx={{ overflow: 'hidden' }}>
                                <Typography variant="overline" sx={{ color: '#64748b', fontWeight: 800, letterSpacing: 1.5, fontSize: '0.65rem', display: 'block', whiteSpace: 'nowrap' }}>QUESTION├üRIOS</Typography>
                ...
                {/* Gr├ífico e Pend├¬ncias Lado a Lado */}
                <Grid item xs={12} md={8.5}>
                    <Paper sx={{ 
                        ...
                        <Typography variant="h6" sx={{ mb: 2, fontWeight: 900, display: 'flex', alignItems: 'center', gap: 1.5, color: '#1b5e20' }}>
                            <ChartIcon sx={{ fontSize: 24 }} /> Fluxo de Question├írios ({selectedYear})
                        </Typography>
                        ...
                        <Box className="carousel-container" sx={{ flex: 1, height: '350px' }}>
                            {pendingUsers.length > 0 ? (
                                ...
                                <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', opacity: 0.6 }}>
                                    <Typography variant="caption" fontWeight={700}>Ô£à Sem pend├¬ncias</Typography>
                                </Box>
                            )}
                        </Box>
                    </Paper>
            ...
            >
                <Typography variant="subtitle2" fontWeight="bold">­ƒøí´©Å Pol├¡tica de Anonimato Ativada (Lotes de {batchSize})</Typography>
                <Typography variant="body2">
                    Para garantir o anonimato dos colaboradores, os resultados e nomes s├úo liberados apenas em grupos de {batchSize} pessoas. 
                    Atualmente existem {realTotalResponses} question├írios respondidos totais, dos quais {stats.responses} j├í foram processados e anonimizados.
                </Typography>
            </Alert>
        </Box>
    );
